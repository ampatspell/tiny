...
import { useDataProperties } from '../properties/data.svelte.ts';
...
import { serialize, type BaseFieldOptions, type Field } from './utils.svelte.ts';
...
export const withData = <D extends Record<string, unknown>>(_opts: OptionsInput<{ data: D }>) => {
  ...
  const define = <R extends Record<string, Field>>(cb: (arg: ReturnType<typeof factory>) => R) => {
    const fields = cb(factory());

    const state = options({
      isDirty: getter(() => properties.isDirty),
      touch: properties.touch,
      rollback: properties.rollback,
      serialized: run(() => {
        const all = $derived(serialize.all(fields));
        const dirty = $derived(serialize.dirty(fields));
        return options({
          all: getter(() => all),
          dirty: getter(() => dirty),
        });
